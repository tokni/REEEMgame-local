...
import { AllstateDataBrowser } from "../../AllEUstateDataPlus2BrowserTimes"
...
export class TKN_Map {
    ...
    private m_colorOfBrowser: ColorPalette;
    ...
    private m_colorOfDefaultBrowser: ColorPalette;

    constructor() {
        var self = this;
        console.log("Constructing TKN_map");
        this.m_geoJSON_data = AllstateData;
        this.m_geoJSON_data_browser = AllstateDataBrowser;
        ...
        this.m_colorOfDefaultBrowser = new ColorPalette('#d73027',
            [{ color: '#fc8d59', value: 10000 },
            { color: '#fee08b', value: 15000 },
            { color: '#ffffbf', value: 20000 },
            { color: '#d9ef8b', value: 30000 },
            { color: '#91cf60', value: 40000 },
            { color: '#1a9850', value: 50000 }
            ]);
        ...
        this.m_global_info.onAdd = function (map) {
            //console.log("This from global_info_onAdd: " + JSON.stringify(this));
            ...
            //this._div_content_path_input = L.DomUtil.create('input', 'eu28_info_content_path_input', this._div_content);
            //this._div_path_comment = L.DomUtil.create('button', 'eu28_info_content_path_comment', this._div_content_path_input);
            ...
            //this._div_content_var_input = L.DomUtil.create('input', 'eu28_info_content_var_input', this._div_content);
            //this._div_var_comment = L.DomUtil.create('button', "eu28_info_content_var_comment", this._div_content_var_input);
            ...
            //$(".eu28_info_content_path_comment").hide();
            //$(".eu28_info_content_var_comment").hide();
            //var t1 = $(".eu28_info_content_path_comment");
            
            
            //this._div_content.id = 'global_info';
            this.update();
            return this._div;
        }
        this.m_global_info.update = function (props) {
            //console.log("This from global_info_update: " + JSON.stringify(this));
            if (props) {
                if (props.path_name) {
                ...
                //if (!this._div_path_comment) {
                //    this._div_content_path = L.DomUtil.create('div', 'eu28_info_content_path', this._div_content);
                //    this._div_content_path.innerHTML = 'Pathway: ' + props.path_name;
                //    this._div_path_comment = L.DomUtil.create('button', 'eu28_info_content_path_comment', this._div_content);
                //    this._div_path_comment.innerHTML = 'Comment on Pathway';
                //    L.DomEvent.on(this._div_path_comment, 'click', self.on_pathway_comment, { pathway: props.path_name });
                //    var tm = 0;
                //} else {
                    
                //    this._div_content_path.innerHTML = 'Pathway: ' + props.path_name;
                //    L.DomEvent.off(this._div_path_comment);
                //    L.DomEvent.on(this._div_path_comment, 'click', self.on_pathway_comment, { pathway: props.path_name });
                //}
                //if (props.var_name) {
                //    if (!this._div_var_comment) {
                //        this._div_content_var = L.DomUtil.create('div', 'eu28_info_content_var', this._div_content);
                //        this._div_content_var.innerHTML = 'Variable: ' + props.var_name;
                //        this._div_var_comment = L.DomUtil.create('button', 'eu28_info_content_var_comment', this._div_content);
                //        this._div_var_comment.innerHTML = 'Comment on Indicator';
                //        L.DomEvent.on(this._div_var_comment, 'click', self.on_variable_comment);
                //    } else {
                ...
                //        this._div_content_var.innerHTML = 'Variable: ' + props.var_name;
                //        L.DomEvent.off(this._div_var_comment);
                //        L.DomEvent.on(this._div_var_comment, 'click', self.on_variable_comment, { variable: props.var_name });
                //    }
                    

                //    $(".eu28_info_content_var").show();
                //    $(".eu28_info_content_var_comment").show();
                //}
                //else {
                //    //this._div_content_var.innerHTML = '';
                //    ////$(".eu28_info_content_var").hide();
                //    //$(".eu28_info_content_var_comment").hide();
                //}
            }
            else {
        ...
        this.m_info.update = function (props) {
            switch (self.m_layer) {
                ...
                case Layer.browser: {
                    this._div.innerHTML = '<h4>' + self.m_variableName + '</h4>' + (props ?
                        '<b>Country: ' + props.name + '</b><br />' + 
                        '<b>Year: ' + self.m_month + '</b><br />' +
                        '<b>Value: ' + Math.round( props.browser) + ' ' + self.m_currentUnit + '</b><br />'
                        : 'Hover over a country');
                }
            }
        };
        ...
        this.m_legend.update = function () {
            ...
            switch (self.m_layer) {
                ...
                case Layer.browser: self.m_colorPalette = self.m_colorOfDefaultBrowser; break;
            ...
            if (self.m_layer != Layer.none) {
                ...
                for (var i = 0; i < colors.length; i++) {
                    ...
                    //legendValue.setAttribute("style", "background-color" + i.color);
                    //legendTable.setAttribute("backgroundColor", '#800026'); &infin;  &#8734;
                    ...
                    //legendTable.insertBefore(legendRow, legendMinRow);
                }
                this.div.appendChild(legendTable);
                $(".overlayLegend").css("margin-bottom", "55px");
                //$(".overlayLegend").css("border", "black solid 5px");
            }
            else {
                this.div.innerHTML = "";
            }
        ...
        this.m_geojsonBrowser = L.geoJson(AllstateDataBrowser, {
    ...
    public setStyleBrowser = (feature) => {
        //var t = this.m_colorPalette.getColor(feature.properties.browser);
        return {
            //fillColor: this.getColorBrowser(feature.properties.browser),
            fillColor: this.m_colorPalette.getColor(feature.properties.browser),
            weight: 2,
            opacity: 1,
            color: 'white',
            dashArray: '3',
    ...
    public setOnEachFeatureBrowser = (feature, layer) => {
        layer.on({
            mouseover: this.highlightFeature,
            mouseout: this.resetHighlightBrowser,
            dblclick: this.onBrowserDblclick,
            click: this.onBrowserClick,
            contextmenu: this.onBrowserContextmenu
        });
    }
    ...
    public resetHighlightBrowser = (e) => {
...
public updateCurrentLayer(p_month: number, p_pathway_name?, p_unit?, var_name?) {
    ...
    public changeDeafultBrowserValues(p_values: number[]) {
        this.m_colorOfDefaultBrowser.changeValues(p_values);
    }
