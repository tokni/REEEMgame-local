...
export default class ModelDev {
    ...
    public changeScenario(p_scenarioID: string): { scenarioData: { roles: Role[], duration: number, time: number, status: GameStatus, score: { c: number, s: number, v: number, o: number } }, historyData: SimulationHistory} {
        this.m_currentScenario.reset()//Reset old scenario to make it ready for next use
    ...
    private executeTick = () => {
        if ( this.m_readyForNextTick) {
                ...
                this.m_currentScenario.getGameLogic().CalculateMonthValues(this.m_time, this.m_time, [dec.East.SubEast, dec.West.SubWest], [dec.East.ResEast, dec.West.ResWest], /*[dec.NewRole.newDec1, dec.NewRole.newDec2]*/);//New role and new dec commented out
    ...
    public calculateEmisionOverlay = (p_eastEmision, p_westEmision) => {
        ...
        this.m_overLayDataPast[0].emisionsCO2 = p_eastEmision; 
        this.m_overLayDataPast[1].emisionsCO2 = p_westEmision; 
    }
